...
async def set_stream_link(client, message):
    try:
        ...
        if link:
            ...
            await message.reply("Usage: /stream https://t.me/BottyUpdates or http://t.me/BottyUpdates")
    except IndexError:
        await message.reply("Usage: /stream https://t.me/BottyUpdates or http://t.me/BottyUpdates")
        return
...
async def m_grp(client, message):
    ...
    if link.text == "/skiplink":
        links.append("https://t.me/BottyUpdates")
    else:
        if checkIfLinkIsValid(link.text):
            links.append(link.text)
        else:
            await message.reply("Invalid link")
    link1 = await client.ask(message.chat.id ,"send me your movies grp link or send /skiplink to skip . default is BottyUpdates")
